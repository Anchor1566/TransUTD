...
class YAMLConfig(BaseConfig):
    ...
    # TODO: add multi-dataloader support
    @property
    def train_multi_dataloader(self, ) -> DataLoader:
        if self._train_multi_dataloader is None and 'train_multi_dataloader' in self.yaml_cfg:
            self._train_multi_dataloader = self.build_dataloader('train_multi_dataloader')
        return super().train_multi_dataloader
    
    @property
    def val_multi_dataloader(self, ) -> DataLoader:
        if self._val_multi_dataloader is None and 'val_multi_dataloader' in self.yaml_cfg:
            self._val_multi_dataloader = self.build_dataloader('val_multi_dataloader')
        return super().val_multi_dataloader
    ...
    def multi_evaluator(self, ):
        if self._multi_evaluator is None and 'evaluator' in self.yaml_cfg:
            if self.yaml_cfg['evaluator']['type'] == 'CocoEvaluator':
                from ..data import get_coco_api_from_dataset
                base_ds = get_coco_api_from_dataset(self.val_multi_dataloader.dataset)                
                self._multi_evaluator = create('evaluator', self.global_cfg, coco_gt=base_ds)
            else:
                raise NotImplementedError(f"{self.yaml_cfg['evaluator']['type']}")
        return super().multi_evaluator
