...
class CocoVideoDetection(torchvision.datasets.CocoDetection, DetDataset):
    ...
    def sample_ref_frames(self, img_id, video_id):
        ref_img_ids = []
        sample_range = []
        min_offset = -(self.num_refs + 1) * 6 
        max_offset = (self.num_refs + 1) * 6
    ...
    def _ref_transforms1(self, ):
        ref_transform = T.Compose([
            # T.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.1),
            T.ColorJitter(brightness=0.1, saturation=0.1),
            # TODO
        ])
        return ref_transform
    
    def _ref_transforms2(self, ):
        ref_transform = T.Compose([
            # T.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.1),
