...
class RTDETRTransformerv2(nn.Module):
    ...
    def _get_decoder_input(self,
        ...
        # TODO: Hack implementation for current frame denoising training
        # memory = memory.reshape(denoising_bbox_unact.shape[0], -1, memory.shape[-1])[:, :memory.shape[1], :]
    ...
    def forward(self, feats, targets=None):
        ...
        current_memory = memory.reshape(len(targets), -1, memory.shape[-1])[:, :memory.shape[1], :]  # memory[::num_frames]
        ref_memory = memory.reshape(len(targets), -1, memory.shape[-1])[:, memory.shape[1]:, :]

        init_ref_contents, init_ref_points_unact, enc_topk_bboxes_list, enc_topk_logits_list = \
            self._get_decoder_input(current_memory, spatial_shapes, denoising_logits, denoising_bbox_unact)
        # get decoder input of the reference frames
        # ref_init_ref_contents, ref_init_ref_points_unact, _, _ = \
        #     self._get_decoder_input(ref_memory, spatial_shapes) 
